...
@interface MWPhotoBrowser () {
    ...
    *_navigationBarBackgroundImageLandscapePhone;
    ...
    BOOL _displayRemovePhotoButton;
}
...
@synthesize displayAddPhotoButton = _displayAddPhotoButton, displayRemovePhotoButton = _displayRemovePhotoButton;
...
- (void)viewDidLoad {
    ...
    if (_displayAddPhotoButton) {
        UIBarButtonItem *btn = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(addPhoto)]; 
        
        self.navigationItem.rightBarButtonItem = btn;
        
        [btn release];
        
    } else if (_displayRemovePhotoButton) {
        UIBarButtonItem *btn = [[UIBarButtonItem alloc] initWithTitle:@"Remove" style:UIBarButtonItemStyleBordered target:self action:@selector(removePhoto)]; 
...
- (void)reloadData {
    ...
    if (_photoCount > 0 && _displayAddPhotoButton && !self.navigationItem.rightBarButtonItem.enabled) {
...
//                                      NSLocalizedString(@"Save", nil), 
                                      NSLocalizedString(@"Copy", nil), NSLocalizedString(@"Email", nil), nil] autorelease];
            } else {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
                                                        otherButtonTitles:
//                                      NSLocalizedString(@"Save", nil), 
                                      NSLocalizedString(@"Copy", nil), nil] autorelease];
            }
...
- (void)addPhoto {
    ...
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([_delegate respondsToSelector:@selector(photoBrowserChosePhotoToBeAddedWithAssetURL:)] && [photo assetURL]) {
        [_delegate photoBrowserChosePhotoToBeAddedWithAssetURL:[photo assetURL]];
    }
}
...
- (void)removePhoto {
    ...
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([_delegate respondsToSelector:@selector(photoBrowserChosePhotoToBeRemovedWithAssetURL:)] && [photo assetURL]) {
        [_delegate photoBrowserChosePhotoToBeRemovedWithAssetURL:[photo assetURL]];
    }
}
