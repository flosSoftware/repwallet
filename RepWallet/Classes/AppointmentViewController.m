...
@synthesize addAnotherAppointment;
...
- (void)showAddForm 
{
    Appointment *evt = [(Appointment *)[[NSManagedObject alloc] initWithEntity:[self.dao getEntityDescriptionForName:@"Appointment"] insertIntoManagedObjectContext:nil] autorelease];
    [evt setInsertDate:[NSDate date]];
    
    AddEditViewController * formViewController = [[AddEditViewController alloc] initWithStyle:UITableViewStylePlain title:ADDEDITVIEWCONTROLLER_INSERT_MODE_TITLE entity:evt andDao:self.dao addAnotherOne:self.addAnotherAppointment];
...
{
    AppointmentForFirmViewController *viewController = [[AppointmentForFirmViewController alloc] initWithStyle:UITableViewStylePlain firm:selectedFirm andDao:self.dao];
    viewController.title = [NSString stringWithFormat:@"Appointments - %@", selectedFirm.firmName];
...
- (id)initWithStyle:(UITableViewStyle)style andDao:(DAO *)aDao
{
    ...
    if (self) {
        ...
        [self disableAnotherAppointmentInsertion];
        ...
        [[NSNotificationCenter defaultCenter] 
         addObserver:self selector:@selector(enableAnotherAppointmentInsertion) 
         name:ENABLE_INSERTION_OF_ANOTHER_APPOINTMENT_NOTIFICATION object:nil];
        
        [[NSNotificationCenter defaultCenter] 
         addObserver:self selector:@selector(disableAnotherAppointmentInsertion) 
         name:DISABLE_INSERTION_OF_ANOTHER_APPOINTMENT_NOTIFICATION object:nil];
        
        UITabBarItem * item = [[UITabBarItem alloc] initWithTitle:@"Appointments" image:[UIImage imageNamed:@"chrono.png"] tag:0];
...
- (void) loadFilteredData:(NSString *)text {
    ...
    [self.filteredFetchedResultsController.fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"appointments.@count > 0 AND firmName CONTAINS[cd] %@", text]];
...
- (void) loadData 
{
    [NSFetchedResultsController deleteCacheWithName:@"FirmsWithAppointments"];
    NSError *error;
...
- (void)viewDidLoad 
{
    ...
    self.fetchedResultsController = [self.dao fetchedResultsControllerForEntityType:NSStringFromClass([Firm class]) withDelegate:self cacheName:@"FirmsWithAppointments"];
    
    [self.fetchedResultsController.fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"appointments.@count > 0"]];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (self.addAnotherAppointment == YES) {
        [self showAddForm];
    } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath 
{
    ...
    if(firm)
        [self showAppointmentsForFirm:firm];
...
{
	self.addAnotherAppointment = YES;
}

- (void)disableAnotherAppointmentInsertion
{
	self.addAnotherAppointment = NO;
}
