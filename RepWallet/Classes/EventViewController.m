...
- (void)showAddForm 
{
    Event *evt = [(Event *)[[NSManagedObject alloc] initWithEntity:[self.dao getEntityDescriptionForName:@"Event"] insertIntoManagedObjectContext:nil] autorelease];
    [evt setInsertDate:[NSDate date]];
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSNumber *taxRate = (NSNumber *)[defaults objectForKey:TAX_RATE_SETTING_KEY];
    [evt setTaxRate:taxRate];
    
    AddEditViewController * formViewController = [[AddEditViewController alloc] initWithStyle:UITableViewStylePlain title:ADDEDITVIEWCONTROLLER_INSERT_MODE_TITLE entity:evt andDao:self.dao addAnotherOne:self.addAnotherEvent];
    [self.navigationController pushViewController:formViewController animated:YES];
    [formViewController release];
}

- (void)showEventsForFirm:(Firm *) selectedFirm 
{
    EventForFirmViewController *viewController = [[EventForFirmViewController alloc] initWithStyle:UITableViewStylePlain firm:selectedFirm andDao:self.dao];
    viewController.title = [NSString stringWithFormat:@"Events - %@", selectedFirm.firmName];
    self.navigationController.toolbar.tintColor =[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0];
...
- (id)initWithStyle:(UITableViewStyle)style andDao:(DAO *)aDao
{
    ...
    if (self) {
        ...
        [self disableAnotherEventInsertion];
        ...
        [[NSNotificationCenter defaultCenter] 
         addObserver:self selector:@selector(enableAnotherEventInsertion) 
         name:ENABLE_INSERTION_OF_ANOTHER_EVENT_NOTIFICATION object:nil];
        
        [[NSNotificationCenter defaultCenter] 
         addObserver:self selector:@selector(disableAnotherEventInsertion) 
         name:DISABLE_INSERTION_OF_ANOTHER_EVENT_NOTIFICATION object:nil];
        
        UITabBarItem * item = [[UITabBarItem alloc] initWithTitle:@"Events" image:[UIImage imageNamed:@"events.png"] tag:0];
...
- (void) loadFilteredData:(NSString *)text {
    ...
    [self.filteredFetchedResultsController.fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"events.@count > 0 AND firmName CONTAINS[cd] %@", text]];
...
- (void) loadData 
{
    [NSFetchedResultsController deleteCacheWithName:@"FirmsWithEvents"];
    NSError *error;
...
- (void)viewDidLoad 
{
    ...
    self.fetchedResultsController = [self.dao fetchedResultsControllerForEntityType:NSStringFromClass([Firm class]) withDelegate:self cacheName:@"FirmsWithEvents"];
    
    [self.fetchedResultsController.fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"events.@count > 0"]];
...
- (void)enableAnotherEventInsertion
{
	self.addAnotherEvent = YES;
}

- (void)disableAnotherEventInsertion
{
	self.addAnotherEvent = NO;
}
