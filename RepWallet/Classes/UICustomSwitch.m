...
-(id)initWithFrame:(CGRect)rect
{
    ...
    if ([(RepWalletAppDelegate *)[[UIApplication sharedApplication] delegate] isIpad]) {
        w = IPAD_SWITCH_WIDTH;
        h = IPAD_SWITCH_HEIGHT;
    } else {
        w = SWITCH_WIDTH;
        h = SWITCH_HEIGHT;
    }
	if ((self=[super initWithFrame:CGRectMake(rect.origin.x,rect.origin.y,w,h)]))
	{
...
-(void)awakeFromNib
{
	...
	self.clippingView = [[UIView alloc] initWithFrame:CGRectMake(4,2,(self.frame.size.width)-8,roundf(self.frame.size.height)-4)];
    ...
    float f;
    if ([(RepWalletAppDelegate *)[[UIApplication sharedApplication] delegate] isIpad]) {
        f = [UIFont labelFontSize] + 10;
    } else
        f = [UIFont labelFontSize];
	
	self.leftLabel = [[UILabel alloc] init];
	self.leftLabel.frame = CGRectMake(0, 0, roundf(0.5*self.frame.size.width), roundf(self.frame.size.height)-4);
	self.leftLabel.text = leftLabelText;
	self.leftLabel.textAlignment = UITextAlignmentCenter;
	self.leftLabel.font = [UIFont boldSystemFontOfSize:f];
	...
	self.rightLabel.frame = CGRectMake(roundf(self.frame.size.width), 0, roundf(0.5*self.frame.size.width), roundf(self.frame.size.height)-4);
	self.rightLabel.text = rightLabelText;
	self.rightLabel.textAlignment = UITextAlignmentCenter;
	self.rightLabel.font = [UIFont boldSystemFontOfSize:f];
...
-(void)layoutSubviews
{
	...
	self.leftLabel.frame = CGRectMake(xPos, 0, labelWidth, roundf(self.frame.size.height)-4);
	...
	self.rightLabel.frame = CGRectMake(xPos, 0, labelWidth, roundf(self.frame.size.height)-4);
